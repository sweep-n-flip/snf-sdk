...
/**
 * `resolveSubject` — the `SNF_SDK_PROHIB_SUBJECT` convention every prohibition test in
 ...
 *
 * `SNF_SDK_PROHIB_SUBJECT` accepts exactly the three values the documented prohibition
 ...
 * PowerShell : `$env:SNF_SDK_PROHIB_SUBJECT = "test/fixtures/prohib/X-violation.ts"; pnpm ... vitest run test/prohibitions/Y.test.ts`
 * Git Bash : `SNF_SDK_PROHIB_SUBJECT=test/fixtures/prohib/X-violation.ts pnpm ... vitest run test/prohibitions/Y.test.ts`
 */

const PACKAGE_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..')

export async function resolveSubject<T = Record<string, unknown>>(defaultPath: string): Promise<T> {
  const envPath = process.env.SNF_SDK_PROHIB_SUBJECT

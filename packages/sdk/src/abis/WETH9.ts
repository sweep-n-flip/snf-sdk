 ...
 * the production AMM client's own ABI, only the exported const renamed, PLUS the
 * `Withdrawal`/`Deposit` events (added later — the production client's copy has only
 * the two functions; `receipt/parseReceipt.ts` needs the events to attribute the
 * native leg of a settled

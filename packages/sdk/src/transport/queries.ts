 ...
 * `snf-client/src/lib/queries/pools.ts`'s `GetPools`/`GetPairById` documents — every
 * deployed subgraph indexes the identical SnF v2 schema, so there is no per-chain
 * branching. `_meta` is added to both (not present in the snf-client source) because
 ...
 *
 * `POOL_INVENTORY_QUERY` is copied verbatim from
 * `snf-drops-registration/src/features/mint-details/genesis/inventory/subgraphQueries.ts`'s
 * `PoolInventory` document. `currency(id:...)` and `_meta` are selected in the SAME
 * document on purpose (R4/R7): asking for the ids and the index's own block in two

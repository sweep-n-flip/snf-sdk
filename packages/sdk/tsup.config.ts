...
// Two entry points, deliberately no Node-vs-browser conditional split (RESEARCH.md
// "Toolchain Research" Open Question #3). `viem` is the only peer and is externalised
...
//
// `checkout` (plan 16 addition — see snf-54-16-SUMMARY.md, Deviations) is a SECOND
// entry pointing at the already-written, already-tested `src/checkout/index.ts`
// (plan 08) — no file under `src/` is touched or created by this addition, only this
// build config and `package.json`'s `exports` map. `createCheckout(plan)` is
// `@sweepnflip/sdk-react`'s ONE dispatch-site dependency (D-04, D-02): the reducer
...
// D-01 methods — but "not on the main barrel" and "unreachable from any package" are
// different things, and the latter was never the intent (plan 09's own SUMMARY:
// "consumed by the (future) sdk-react package's useSnfCheckout, a different entry
